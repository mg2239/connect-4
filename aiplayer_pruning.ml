...
let rec minimax minmaxtree depth alpha beta a_or_not : int = 
  let state = match get_state_c minmaxtree with
    | (st, _) -> st in 
  let children = match get_state_c minmaxtree with
    | (_, c) -> c in
  if depth == 0 then Board.score state.board
  else if a_or_not 
  then max_children children (-10000) alpha beta a_or_not
  else
    min_children children 10000 alpha beta a_or_not

and max_children (max_c:minmaxtree list) (maxEval:int) alpha beta a_or_not =
  match max_c with
  | [] -> maxEval
  | h::t -> let eval_hd = minimax h (depth-1) alpha beta (not(a_or_not)) in
    if not (beta > alpha) then maxEval
    else
      let eval_tl = max_children t maxEval (max alpha maxEval) beta (not(a_or_not)) in
      let max_child = max eval_hd eval_tl in 
      max maxEval max_child

and min_children (min_c:minmaxtree list) (minEval:int) alpha beta a_or_not =
  match min_c with
  | [] -> minEval
  | h::t -> let eval_hd = minimax h (depth-1) alpha beta a_or_not in
    if not (beta < alpha) then minEval
    else
      let eval_tl = min_children t minEval alpha (min beta minEval) a_or_not in
      let min_child = min eval_hd eval_tl in
      min minEval min_child



(** returns a state after the AI makes a move *)
let make_move_ai (st:State.t) : State.result = 
  State.go (minimax (generate_minmax_tree st) depth (-10000) 10000 true) st
